				...
				if (objResponseWS.flag != false && objResponseWS.model != null) {
					...
					winAddQuotationTwo.addEventListener("open", function(evt) {
						...
						if (Ti.Platform.osname === "android") {
							...
							// Activity
							var activityAddQuotationTwo = winAddQuotationTwo.activity;

							if (!activityAddQuotationTwo) {
								Ti.API.error("No se puede acceder a la barra de acción en una ventana ligera.");
							} else {

								// Action Bar de la ventana
								actionBar = winAddQuotationTwo.activity.actionBar;
					...
					Ti.UI.createAlertDialog({
						message : '¡El modelo no existe!. Intentalo de nuevo.',
						title : 'Modelo',
						ok : 'Aceptar',
					}).show();
				};
