...
function addQuotation(objDataConveyor) {
	...
	winAddQuotation.addEventListener("open", function(evt) {
		...
		if (Ti.Platform.osname === "android") {
			...
			// Activity
			var activityAddQuotation = winAddQuotation.activity;
			
			if (! activityAddQuotation) {
				Ti.API.error("No se puede acceder a la barra de acción en una ventana ligera.");
			} else {
				
				// Action Bar de la ventana
				actionBar = winAddQuotation.activity.actionBar;
				...
				if (actionBar) {
					...
					// Agregamos un titulo
					actionBar.title = "Detalles del modelo";
